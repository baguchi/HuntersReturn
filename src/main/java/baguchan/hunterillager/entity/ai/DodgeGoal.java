...
public class DodgeGoal extends Goal {
	...
	public DodgeGoal(Hunter hunterEntity, Class<? extends Projectile> dodgeAtType) {
		...
		this.lookAtContext = TargetingConditions.forNonCombat().range((double) 8.0F);
	}
	...
	public boolean canUse() {
		if (--this.cooldownTime < 0) {
			if (this.hunter.getTarget() != null) {
				this.toAvoid = this.hunter.level.getEntitiesOfClass(this.dodgeAtType, this.hunter.getBoundingBox().inflate((double) 8.0F, 3.0D, (double) 8.0F), (p_148124_) -> {
					return (!(p_148124_ instanceof AbstractArrow) || !((AbstractArrow) p_148124_).isOnGround()) && p_148124_.getOwner() != this.hunter && (p_148124_.getOwner() == null || !this.hunter.isAlliedTo(p_148124_.getOwner()));
				});
	...
	public void tick() {
		super.tick();
		--this.dodgeTime;
		this.hunter.getMoveControl().strafe(-0.85F, this.dodgeDirection ? 0.85F : -0.85F);
	}
